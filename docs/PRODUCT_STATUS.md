...
| **Wallboard** | **v2 shipped (#430):** session-cookie token storage, responsive tile grid, auto-rotation, stack-aware KEV tile, mono terminal styling. Token via `X-BRIEFR-Wallboard-Token` header or admin config `WALLBOARD_TOKEN` (#514 — Security / kiosk section on API keys & config). |
