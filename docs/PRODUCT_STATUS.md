...
| **UI design system (UI-M)** | **Phase 0 E0-1/E0-2/E0-3 shipped:** `tokens.css` wired; semantic tokens + legacy aliases; design-token lint in `verify-local.sh`; tool-wide motion toggle (`data-motion`). **Radix Checkbox** primitive (`components/ui/Checkbox.jsx`) adopted with reference use on Admin scheduler job table (E0-2); more primitives in E3. **Reliability (E1-1):** correlation snapshot precompute behind `CORRELATION_PRECOMPUTE_ENABLED` (default off) — nightly job writes `correlation_cve_snapshot`; request path reads when flag on; hub IOC degree cap in shared-IOC SQL. |
