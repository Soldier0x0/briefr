...
| **UI design system (UI-M)** | **Phase 0 E0-1/E0-2 shipped:** `tokens.css` wired; semantic tokens + legacy aliases; design-token lint in `verify-local.sh`. **Radix Checkbox** primitive (`components/ui/Checkbox.jsx`) adopted with reference use on Admin scheduler job table (E0-2); more primitives in E3. |
