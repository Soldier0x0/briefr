...
| **UI (Track E)** | Drawer Intel: compact correlation (E-PR2); exploits table + EPSS flat-line rule (E-PR3); **GreyNoise on-demand** with weekly quota UI — not auto on drawer open (E-PR4). BRIEF tab: E1 states audit complete (E-PR5) — heatmap empty state + `brief-intel-row` layout fix. |
