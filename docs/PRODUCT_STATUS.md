...
| Area | Status |
|------|--------|
| **Database** | **PostgreSQL required** (`DATABASE_URL`, `BRIEFR_REQUIRE_POSTGRES=1`). SQLite removed from production path. **Intel snapshot:** `scripts/export_intel_snapshot.py` exports allowlisted tables per `docs/DATA_SNAPSHOT.md`; Postgres CI runs export→restore smoke (`test_intel_snapshot_export.py`). |
...
| **CVE Overview (analyst workflow)** | Overview tab reordered for analyst decisions: score hero → asset exposure status → threat signals → remediation block → risk/exploit sections → methodology collapsed under **WHY THIS SCORE?** Asset exposure shows **EXPOSURE UNKNOWN** when no profile (no misleading 50% bar). Investigation actions renamed **Start investigation** / **Extract observables**. Feed distinguishes opened CVE vs investigation vs bulk-select. |
