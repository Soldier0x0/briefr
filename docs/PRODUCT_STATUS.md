...
| **UI design system (UI-M)** | **Phase 0 E0-1 shipped:** `frontend/src/styles/tokens.css` imported before `App.css` from `main.jsx`; semantic token layer (severity, status, spacing, motion, `--accent-selected`) with legacy `--bg`/`--text`/`--type-*` aliases; design-token lint gates in `./scripts/verify-local.sh`. Radix primitives not yet adopted (E0-2). |
