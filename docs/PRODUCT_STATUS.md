...
| TM-1: Security Architecture Corpus generator + loader + drift CI, `/api/security-architecture/{manifest,overview}` stub | TM-4: System Architecture graph, Trust Boundaries, Attack Surface |
| TM-2: `/security-architecture` shell + Overview — ARCH header tab, manifest-driven nav, drill-through evidence tiles, generic `/api/security-architecture/section/{id}` stub reads, simplified architecture stack | TM-5: Risk Register grid, Decision records, Review History, Abuse Cases, global search, PDF export |
| TM-3: MITRE ATT&CK live coverage matrix (`GET /api/security-architecture/mitre`, reuses `routers.forge.build_coverage_map`), Threat Scenarios wrap (`GET /api/security-architecture/threat-scenarios`, reuses `build_threat_scenarios()`, 3 catalogs: operational/stack/self-stack), Controls inventory seeded + live `active` flags from runtime env vars, Self-exposure (§4.5): generated `self_stack.yaml` (deps from `requirements.txt`/`package.json`), live risk-register rows (`origin: live`) from KEV/critical CVE hits on the self-stack with matched term shown, Overview tiles for MITRE coverage + Self CVE Exposure | TM-6+: STRIDE/OWASP/CAPEC/CWE/NIST CSF/ASVS framework workspaces (evidence-gated) |
