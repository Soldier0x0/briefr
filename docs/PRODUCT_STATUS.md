...
| **LLM router (K1–K3)** | Scheduler-side multi-provider router: Groq (`openai/gpt-oss-20b` / `120b` for PDF) → Gemini Flash-Lite → Cerebras → OpenRouter `:free`; product extraction + PDF executive summary wired through router; Anthropic removed from chain; `feed_cache` provenance `{provider, model}`. **AI operations (AI-1, #416):** each `chat_completion_task` attempt records a redacted row in `ai_operations` (no prompt text); model failover chains centralized in `ai/model_catalog.py`; disable recording with `AI_OPERATIONS_RECORD=0`; admin `GET /api/admin/ai/operations/models` exposes the catalog. **Token usage (#420):** when a provider reports usage (OpenAI-compatible `usage`, Gemini `usageMetadata`), `input_tokens`/`output_tokens`/`total_tokens` are stored; Usage and Activity tabs show rollups and per-row counts; `estimated_cost_usd` stays NULL (no price SSOT — AI-3 automation gated). **AI Operations admin page (AI-2, #417; #419):** Operator → **AI operations** — read-only Overview / Providers / Models / Usage / Activity tabs backed by `/api/admin/ai/operations/*`; Activity tab filters by task class and provider. |
...
| Correlation v2 core + phase-4 tail (#364, #389) | STIX export (excluded from current loop) |
| Admin ops, webhooks (#413–#415), wallboard, AI ops (#416–#420) | **PR13** read-only DB explorer |
| **Forge** threat scenarios, proof bench, KEV backlog, IOC watchlist (V1.5 #373–#376) | **F2** AGPL + CONTRIBUTING + FUNDING |
| Track I performance (#378–#382) | **G0** LEARNING_PATH / ONBOARDING refresh |
| Track L Wave 4: monitor alerts, onboarding, doctor, operator settings (#366–#372) | Tracks **K5 / M / N / O** (discussion — see operator backlog §I) |
| Chart.js admin ops dashboard, logrotate deploy artifacts | **AI-3** routing automation (data-gated) |
| Architecture diagrams (phase A) | Track I Phase 3 perf; embeddings; `IMAGE_BRIEFS` tail; MkDocs |
