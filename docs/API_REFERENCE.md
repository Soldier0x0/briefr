...
**Client note:** The SPA must share one in-flight refresh (`frontend/src/api.js` `refreshAccessToken`) and serialize rotation across tabs (`navigator.locks` on HTTPS/localhost, otherwise a `localStorage` mutex). Concurrent `POST /api/auth/refresh` after rotation trips reuse detection and revokes every session for the user (including Remember me).
...
Scheduled jobs: `daily_brief_eod` (cron at EOD hour/minute, instance timezone) and `daily_brief_standup` (cron at standup hour/minute). EOD window = prior 24h. Standup uses `sync_state` `daily_brief:last_eod_end` only while EOD is enabled; otherwise it uses the prior 12h. A watermark older than 24h is clamped to `window_end − 24h`. Standup skips when the computed window is shorter than 15 minutes (overlap guard after EOD). SQL timestamp bounds are UTC and half-open; date-only KEV rows use `date_added > start_local_date AND date_added <= end_local_date`. Structured `brief` includes `market`, `headlines` (`[{source, title, url}]`, max 3 from `incident_feed:snapshot`), and `advisories` (same shape, max 2 from `publications`). Discord delivery uses `discord_embeds`; Telegram uses `parse_mode=HTML`. Message layout: [`docs/design/daily-brief-format.md`](design/daily-brief-format.md). Optional LLM rewrites only the headline when `DAILY_BRIEF_LLM_ENABLED=1` and a provider is configured; template headline otherwise.

### GET /api/admin/webhooks/daily-brief/preview
Params: `slot` — required; `eod` or `standup` (else `422`).

Builds daily-brief copy for the slot **without dispatching** (no delivery log, no dedupe claim). Response: `{text, html, discord_embeds, brief}` — `text` is the human-readable fallback body; `html` is Telegram HTML; `discord_embeds` is the Discord embed list; `brief` is the structured fact object (`slot`, window bounds, counts, list rows, `market`, `headlines`, `advisories`, `lede_source`). Works while `DAILY_BRIEF_*_ENABLED` cron flags are off; bypasses standup overlapping-window skip so operators can inspect copy anytime. Admin UI: `/admin?p=dailybrief`.
...
Dispatches a real `daily_brief` webhook event with **`skip_dedupe=True`** (repeatable operator test). Works while cron slot flags are off. Response merges dispatch result (`status`, `sent`, `errors`, …) with `{slot, text, html, discord_embeds, brief}`. Generic HTTPS payloads include structured `brief` (and `discord_embeds`) in `payload_extra`. Audit: `webhook.daily_brief.test.{slot}`.
