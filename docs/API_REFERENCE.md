...
**Client note:** The SPA must share one in-flight refresh (`frontend/src/api.js` `refreshAccessToken`) and serialize rotation across tabs (`navigator.locks`). Concurrent `POST /api/auth/refresh` after rotation trips reuse detection and revokes every session for the user (including Remember me).
