...
**Notes:** KEV-on-stack webhooks (`kev_alert`) match the latest active **admin** My Stack (profile assets preferred, else `stack_terms` as product names) against CVE `cpe_matches` / structured `affected_products` — not description LIKE and not `BRIEFR_STACK_TERMS`. The FEED `stack` query param remains a throwaway keyword filter. `BRIEFR_STACK_TERMS` still drives wallboard keyword tiles; it is not a last-user fallback for alerts.
