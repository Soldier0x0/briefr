...
- **Frontend refresh dedupe (#731):** `frontend/src/api.js` keeps a module-level in-flight `refreshAccessToken()` promise so concurrent 401 handlers share one rotation. `AuthContext` bootstrap calls `fetchMe()` only (`request()` already refreshes on 401) — never a bare `fetch('/api/auth/refresh')`, which raced retries and tripped reuse detection (sticky BRIEF “Not authenticated” while the header still showed the user). **Cross-tab:** `navigator.locks.request('briefr-auth-refresh')` plus a `GET /api/auth/me` probe so a second tab does not replay the pre-rotation refresh cookie (that path revoked all sessions and ignored Remember me).
