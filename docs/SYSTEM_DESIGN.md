...
└──────────────────────────────────┬──────────────────────────────────────────┘
                                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│ PostgreSQL 16 + asyncpg pool (pgvector). DATABASE_URL is required.          │
└──────────────────────────────────┬──────────────────────────────────────────┘
                                   ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
Flowchart: [`docs/diagrams/startup.mermaid`](diagrams/startup.mermaid) (scheduler registration)
...
6. **Forge redesign FR-1/FR-2 (2026-07):**
...
3. **Request path — related (E3):** `GET /api/cves/{id}/related` does **no model inference**. Prefers ANN / cosine on `embeddings` (pgvector), then legacy `cve_embeddings` NumPy, then product heuristic. `meta.method` is `embeddings` or `product_heuristic`.
...
- **Frontend refresh dedupe (#731):** `frontend/src/api.js` keeps a module-level in-flight `refreshAccessToken()` promise so concurrent 401 handlers share one rotation. `AuthContext` bootstrap calls `fetchMe()` only (`request()` already refreshes on 401) — never a bare `fetch('/api/auth/refresh')`, which raced retries and tripped reuse detection (sticky BRIEF “Not authenticated” while the header still showed the user). **Cross-tab:** `navigator.locks.request('briefr-auth-refresh')` when available (secure context), else a `localStorage` mutex, plus a `GET /api/auth/me` probe (`cache: no-store`) so a second tab does not replay the pre-rotation refresh cookie (that path revoked all sessions and ignored Remember me).
...
- **What:** backup archives (Postgres `pg_dump` + `.env` with all API keys + manifest) are encrypted to the age format (X25519, via `pyrage` — interoperable with the `age` CLI) and named `briefr-*.tar.gz.age`. The identity file is `BACKUP_AGE_KEY_FILE` (production default `/var/lib/briefr/keys/backup-age.key`, generated by `deploy/briefr-backup.sh` / `python -m backup keygen` on first run, mode 0600). Legacy archive restore can still unpack an old `briefr.db` SQLite file from pre-Postgres backups.
...
- **Scope honesty:** this protects **off-site / at-rest archive copies only** (rclone/S3, stolen disks, leaked archive directories). A compromised host or service user can read the key.
...
- **KEV-on-stack:** after each `kev_metadata_sync`, newly flagged KEV CVEs that match the admin My Stack via CPE / `affected_products` dispatch `kev_alert` (deduped per CVE). Description keyword matching is not used for this alert.
...
- **Why:** Production and local runtime require `DATABASE_URL` (`postgresql://`). Runtime uses an asyncpg pool, Alembic migrations, pgvector embeddings, db-backed rate-limit buckets, resource metrics, durable Procrastinate jobs, API metering, and Postgres backup/restore smoke tests.
...
- **SQL twins:** Some `backend/db/*` modules still carry `_SQLITE` / `_PG` query variants and `pg_adapt.py` (`?` / `datetime('now')`) for historical dual-dialect sources. The live connection is always Postgres.
...
- **PostgreSQL 16 + pgvector is required.** There is no SQLite runtime. Production and local installs use `DATABASE_URL=postgresql://…`.
