...
**Status (2026-07-21):** **PLANNING ONLY — DO NOT IMPLEMENT** while another session is actively landing PRs on `main`. This document is the RCA + task plan; execution starts only when the maintainer explicitly says to execute, after a fresh `git pull origin main` and re-verify of anchors below.
