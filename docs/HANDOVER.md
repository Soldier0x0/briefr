...
| TBD | vulnrichment-cvelistv5-feeds | V1.3 T2-S3: `feeds/vulnrichment.py` (CISA ADP snapshot, gap-fill CVSS/CWE/CPE) + `feeds/cvelistv5.py` (GitHub compare deltas, `sync_state.cvelistv5_head_sha`); additive merge in `database.apply_additive_cve_enrichments`; scheduler jobs `vulnrichment_snapshot_sync` + `cvelistv5_incremental_sync`; `feeds.sources` health keys `vulnrichment` / `cvelistv5` | 🔲 Open |
