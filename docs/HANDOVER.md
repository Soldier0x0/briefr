...
| #106 | change-intel-kev-countdown | V1.3 tranche 2: What changed panel (`GET /api/changes`, field + 24h/48h/7d filters, row → drawer); KEV **Due in N days** chip on cards (`kev_due_date` additive on list/export/detail); sidebar deadlines `sort=urgent` | 🔲 Open |
