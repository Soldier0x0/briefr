...
| TBD | playwright-ci-smoke | §5.7: Chromium-only Playwright smoke in `backend-tests.yml` (third job) against `scripts/seed_screenshot_data.py` — BRIEF cards, filter→feed anchor, drawer focus restore, IOC input, Incidents cards (`tests/test_playwright_smoke.py`, skipped unless `PLAYWRIGHT_SMOKE=1`) | 🔲 Open |
