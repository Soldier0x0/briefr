...
| TBD | cursor/backup-age-encryption-2555 | §5.6: age (X25519) archive encryption via `pyrage` — `briefr-*.tar.gz.age`; `BACKUP_AGE_KEY_FILE` (default `/var/lib/briefr/keys/backup-age.key`, generated by `briefr-backup.sh` first run) enforced **outside** `BACKUP_DIR`; `python -m backup keygen`; restore/auto-restore decrypt both old plaintext and new encrypted archives; round-trip + auto-restore pytest coverage (`tests/test_backup_encryption.py`) | 🔲 Open |
