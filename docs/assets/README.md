# Exported documentation images

Place **PNG** (2×) or **SVG** files here. Filenames must match [`IMAGE_BRIEFS.md`](../IMAGE_BRIEFS.md).

| File | Status |
|------|--------|
| `briefr-docs-tokens.css` | Shared dark palette for generated doc surfaces (mirrors product tokens) |
| `placeholder-diagram.svg` | Generic placeholder (used until real assets exist) |
| `production-architecture.svg` | Phase A shipped |
| `auth-layers.svg` | Phase A shipped |
| `correlation-pipeline.svg` | Phase A shipped |
| … | See IMAGE_BRIEFS checklist |

**Workflow:** Miro → Export → save here → update doc `![...]()` path from `placeholder-diagram.svg` to your file.

**Docs UI:** edit `briefr-docs-tokens.css` when syncing palette changes to the public docs portal.
