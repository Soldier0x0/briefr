...
function unwrapExecutiveSummaryText(text) {
  const trimmed = String(text || '').trim()
  if (!trimmed.startsWith('{')) return trimmed
  try {
    const parsed = JSON.parse(trimmed)
    if (parsed && typeof parsed === 'object') {
      const inner = parsed.executive_summary || parsed.summary
...
export function formatExecutiveSummaryBody(summaryData) {
  ...
  const parts = [unwrapExecutiveSummaryText(summaryData.executive_summary)]
