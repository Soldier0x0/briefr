...
function drawCodeHeader(doc, title, x, y0, w, accentRgb) {
  if (accentRgb) {
    doc.setFillColor(...accentRgb)
    doc.rect(x, y0, 2.5, CODE_HEADER_H + CODE_PAD_TOP, 'F')
  }
...
export function drawPdfCodeBlock(ctx, title, code, accentRgb, layout) {
  ...
  while (lineIndex < lines.length) {
    const available = layout.contentBottom != null
      ? layout.contentBottom - ctx.y - CODE_HEADER_H - CODE_PAD_TOP - CODE_BOTTOM_PAD - 6
      : 220
    ...
    const blockH = (firstChunk ? CODE_HEADER_H + CODE_PAD_TOP : CODE_PAD_TOP)
    ...
    if (firstChunk) {
      drawCodeHeader(doc, title, x, y0, w, accentRgb)
    }

    let y = y0 + (firstChunk ? CODE_PAD_TOP + CODE_HEADER_H : CODE_PAD_TOP)
    chunk.forEach((line) => {
