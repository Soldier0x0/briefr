...
def build_synced_content(example_path: Path, env_values: dict[str, str]) -> str:
...
def main() -> int:
    ...
    existing_text = args.env.read_text() if args.env.exists() else ""
    env_values = parse_env_values(args.env)
    new_text = build_synced_content(args.example, env_values)

    new_keys = sorted(
        k for k in re.findall(r"(?m)^[A-Za-z_][A-Za-z0-9_]*(?==)", new_text)
        if k not in env_values
    )
    orphan_keys = sorted(set(env_values) - set(
        re.findall(r"(?m)^[A-Za-z_][A-Za-z0-9_]*(?==)", args.example.read_text())
    ))
    ...
    print(f"synced {args.env}: kept {len(env_values) - len(orphan_keys)}, "
