"""LLM product extraction for NVD-unanalyzed CVEs â env-gated (V1.3).
...
and fills ``affected_products`` â ONLY while that field is empty â marking
...
Copyright Â© 2026 Sai Harsha Vardhan. All rights reserved.
"""
...
def _extract_items(text: str) -> list:
    """First candidate that parses AND carries a products list wins â a
...
def parse_products_payload(content: str) -> list[dict]:
    ...
    dicts. Returns [] for anything malformed â never raises."""
...
async def extract_products_via_groq(description: str, api_key: str) -> list[dict]:
    """One Groq call â validated product dicts. retries=0: never burn quota
...
async def run_llm_product_extraction(db: aiosqlite.Connection | None = None, progress_cb=None) -> dict:
    ...
    *completed* extraction (including ones that found no products) is
    ...
    affected_products_source='llm'. Errors are never cached â transient
    failures retry on the next run.

    When ``db`` is omitted, pool connections are held only for short read/write
    scopes â Groq HTTP and throttle sleeps run without a connection.
    """
