...
5. **Scope boundary:** risk register, decision records, review history, abuse cases, global search, and PDF export (TM-5) remain out of this phase — spec `threat-modeling-security-architecture.md` §8 phases them explicitly. TM-4 also documented one spec staleness in `manifest.yaml`'s notes: spec §2.2's navigation catalog uses kebab-case section ids (`system-architecture`, `trust-boundaries`, `attack-surface`); code has used snake_case since TM-1 and TM-4 kept that convention rather than rewriting the established ids.
