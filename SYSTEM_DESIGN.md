...
### H. Security Architecture — corpus + shell (TM-1/TM-2)

1. **Corpus (TM-1):** `backend/security_architecture/corpus/` — versioned YAML, `origin: generated | curated` per record. Generated files (`components.yaml`, `api_inventory.yaml`, `scheduler_jobs.yaml`, `db_tables.yaml`) are emitted by `scripts/generate_security_corpus.py` from live route/scheduler/schema introspection and drift-tested in CI (`test_security_architecture_corpus.py::test_committed_corpus_has_no_drift`). Curated files (`risks.yaml`, `controls.yaml`, `abuse_cases.yaml`, `threat_scenarios.yaml`, `security_decisions.yaml`, `trust_boundaries.yaml`, `reviews.yaml`) are seeded empty pending a real security-review pass. `corpus_loader.py` validates required fields, `origin`, and cross-file `related_ids` at load time (mtime-cached).
2. **API (`routers/security_architecture.py`):** `GET /manifest` returns the corpus version + `sections[]` (the nav source of truth). `GET /overview` returns raw counts plus a `tiles[]` array — every tile value is a `len()` or exact field match over corpus rows (no scoring, spec's "no arithmetic invented" rule), each carrying a `section`/`filter` drill target. `GET /section/{id}` (TM-2) is a generic read of any manifest section's rows with `type`/`status`/`severity`/`stale` filters — a shell convenience superseded by typed endpoints (graph, MITRE, STRIDE, ...) as TM-3+ ships live sections.
3. **UI (TM-2, `frontend/src/pages/security-architecture/`):** `/security-architecture` route, header tab **ARCH** (lazy-loaded like Forge). Three-panel shell mirrors Forge's FR-2 pattern and Admin's `?p=` URL-state convention: left nav renders from `GET /manifest`'s `sections[]` (not hardcoded — a future section appears the moment the corpus lists it), center workspace is `OverviewSection` (evidence tiles + a simplified architecture-stack view built from the generated layer only — no invented "Frontend" node, since `components.yaml` only has backend routers today) or `GenericSection` (stub table for every other section, reading `GET /section/{id}`), context rail stays in an empty state (component/technique/risk detail is TM-3+). All selection state — `?section=&type=&status=&severity=` — round-trips through the URL.
4. **Scope boundary:** MITRE coverage, self-stack CVE exposure (needs TM-3's self-stack generation + `merge.py`), and endpoint↔control attack-surface linkage (TM-4) are deliberately absent from TM-2's tiles and nav — spec `threat-modeling-security-architecture.md` §8 phases them explicitly; faking them early would violate the corpus's honesty-over-confidence design goal.
