...
### H. Security Architecture — corpus + shell + live sections (TM-1/TM-2/TM-3)

1. **Corpus (TM-1, extended TM-3):** `backend/security_architecture/corpus/` — versioned YAML, `origin: generated | curated | live` per record. Generated files (`components.yaml`, `api_inventory.yaml`, `scheduler_jobs.yaml`, `db_tables.yaml`, `self_stack.yaml`) are emitted by `scripts/generate_security_corpus.py` from live route/scheduler/schema/dependency-manifest introspection and drift-tested in CI (`test_security_architecture_corpus.py::test_committed_corpus_has_no_drift`). `self_stack.yaml` (TM-3, spec §4.5) holds one record per dependency term parsed from `backend/requirements.txt` and `frontend/package.json`, plus declared runtime components (`postgresql`, `nginx`) — a new dependency changes this file and fails the drift check until regenerated. Curated files: `controls.yaml` got its first real security-review seed in TM-3 (10 controls: JWT session, bcrypt, rate limiting, parameterized SQL, webhook signing + SSRF guard, backup encryption, Postgres-required, log redaction, audit log); `risks.yaml`, `abuse_cases.yaml`, `threat_scenarios.yaml`, `security_decisions.yaml`, `trust_boundaries.yaml`, `reviews.yaml` are still seeded empty pending a future pass. `corpus_loader.py` validates required fields, `origin`, and cross-file `related_ids` at load time (mtime-cached). `live` rows (self-stack risk-register entries) are never stored in a file — computed at read time by `security_architecture/merge.py` and merged into `/section/risks` responses.
2. **API (`routers/security_architecture.py`):** `GET /manifest` returns the corpus version + `sections[]` (the nav source of truth; includes `mitre_attack`, which has no corpus file — it's served entirely from live DB data). `GET /overview` returns raw counts plus a `tiles[]` array — every tile value is a `len()`, exact field match, or a visible ratio over corpus/live rows (no scoring, spec's "no arithmetic invented" rule), each carrying a `section`/`filter` drill target; TM-3 adds `mitre_detection_coverage` (`"<covered>/<total>"`) and `self_cve_exposure` (live count) tiles. `GET /mitre` (TM-3) reuses `routers.forge.build_coverage_map` — extracted from the Forge coverage-map handler so both endpoints run the identical query, not two implementations that can drift apart. `GET /threat-scenarios` (TM-3) wraps `threat_model.scenarios.build_threat_scenarios` with a `self_stack=true` toggle that swaps in the generated self-stack terms instead of a user `stack` param — same function, same output shape as `/api/threat-model/scenarios`, no duplicated matching logic. `GET /section/{id}` (TM-2, extended TM-3) is a generic read of any manifest section's rows with `type`/`status`/`severity`/`origin`/`stale` filters; TM-3 enriches `controls` rows with a live `active` flag (`merge.resolve_control_active`, reading each control's `live_flag` env var against the runtime, defaulting structural controls with no flag to `true`) and `risks` rows with live self-stack entries (`merge.self_stack_risk_rows`) alongside the curated register. Still a shell convenience superseded by typed endpoints (graph, STRIDE, ...) as TM-4+ ships the remaining live sections.
3. **UI (TM-2 shell, TM-3 live sections, `frontend/src/pages/security-architecture/`):** `/security-architecture` route, header tab **ARCH** (lazy-loaded like Forge). Three-panel shell mirrors Forge's FR-2 pattern and Admin's `?p=` URL-state convention: left nav renders from `GET /manifest`'s `sections[]`. `overview` → `OverviewSection`; `mitre_attack` → `MitreSection` (TM-3: coverage list grouped by tactic, stack-filter input, technique links out to `/?view=coverage&technique=<id>` which activates the Forge tab and selects the technique — reuses Forge's existing `?view=&technique=` URL-state contract, no new deep-link mechanism); `threat_scenarios` → `ThreatScenariosSection` (TM-3: three-catalog toggle — operational/curated via the generic section read, "your stack" reading `GET /api/me/stack` per spec §5.10, "BRIEFR self-stack" needing no client input at all); every other section still renders `GenericSection` (now origin-filterable, and renders `active`/`matched_term` badges when a row carries them). Context rail stays in an empty state — selection wiring is TM-4+. All state — `?section=&type=&status=&severity=&origin=` — round-trips through the URL.
4. **MITRE matrix scope narrowing (TM-3, documented deviation):** spec §5.6 names an `AttackNavigatorMatrix` with 5 coverage layers (Detection/Correlation/YARA/Threat feed/AI). Only Detection has a live data source in this codebase (hunt packs + bundled templates, same as Forge). TM-3 ships a dense grouped-by-tactic list with the one real layer rather than fabricate the other four — inventing rows for layers with no backing data would violate the corpus's central principle. The heat-matrix visualization is a future enhancement, not blocked on anything TM-3 shipped.
5. **Scope boundary:** system architecture graph, trust boundaries, endpoint↔control attack-surface linkage (TM-4), and risk register/decisions/review-history/search/PDF export (TM-5) remain out of this phase — spec `threat-modeling-security-architecture.md` §8 phases them explicitly.
