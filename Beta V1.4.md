...
**Admin gating:** separate Cloudflare Access policy on `/admin/*` (operator email only) + app trusts the identity header for audit; admin routes fail closed in production when no identity is present.
